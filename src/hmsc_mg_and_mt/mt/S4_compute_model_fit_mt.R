...
MFDir = file.path(localDir, "src/hmsc_mg_and_mt/mt/model_fit")
...
filename <- file.path(MFDir, paste("MF_MT_",
...
# Using 2-fold CV: samples randomly assigned to folds
set.seed(1)
...
filename <- file.path(MFDir, paste("MF_CV1_MT_",
                                   "chains_", as.character(nChains),
                                   "_thin_", as.character(thin),
                                   "_samples_", as.character(samples),
                                   ".rds", sep = ""))
...
mean(MF_CV_1$R2,na.rm = TRUE)

# Using 2-fold CV: each fold contains the samples from one trial
set.seed(1)
...
filename <- file.path(MFDir, paste("MF_CV2_MT_",
                                   "chains_", as.character(nChains),
                                   "_thin_", as.character(thin),
                                   "_samples_", as.character(samples),
                                   ".rds", sep = ""))
